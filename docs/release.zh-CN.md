...
release `0.1.9` 的目标为 `community-local-trust`：
...
prerelease。`0.1.9` 已通过该组件门禁，其 stable GitHub release 已发布。root composition
...
兼容版本是准确约束：wrapper/Bridge/Device Client `0.1.9`、协议
...
`0.1.9` 已具备完整的 `community-local-trust` 组件证据。这不代表已经通过 Apple notarization、
