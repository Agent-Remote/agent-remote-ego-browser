...
The stable Bridge `0.1.8` release records `production_ready=true`,
...
| --- | --- |
| Bridge, Device Client, remote wrapper | `0.1.8` |
...
Install the stable `0.1.8` macOS release from its archive, strict aggregate manifest, both Sigstore bundles, and an independently obtained signing-certificate SHA-256:

```sh
./installer/install-macos.sh \
  --archive agent-remote-ego-browser-macos-universal-0.1.8.tar.gz \
  --archive-sigstore-bundle agent-remote-ego-browser-macos-universal-0.1.8.tar.gz.sigstore.json \
  --manifest agent-remote-ego-browser-0.1.8.release-manifest.json \
  --manifest-sigstore-bundle agent-remote-ego-browser-0.1.8.release-manifest.json.sigstore.json \
...
The workflow publishes a stable release only when its generated manifest has `production_ready=true` with no readiness blockers; otherwise it publishes a prerelease. Release `0.1.8` passed that component gate under the `community-local-trust` profile. See [Release, upgrade, and rollback](docs/release.md) for the remaining deployment/canary gates and immutable rollback contract.
