...
This source tree targets `0.1.18`. The tag-bound release manifest determines
...
| --- | --- |
| Bridge, Device Client, remote wrapper | `0.1.18` |
...
The manual archive command is a low-level release/operator path. Install the published `0.1.18` macOS release from its archive, strict aggregate manifest, both Sigstore bundles, and an independently obtained signing-certificate SHA-256:
