...
当前源码树面向 `0.1.19`。组件是否就绪以绑定 tag 的 release manifest 为准；生产环境能否
...
| 范围 | 要求 |
| --- | --- |
| Bridge、Device Client、远端 wrapper | `0.1.19` |
...
下面的手动 archive 命令是底层 release/operator 入口。安装已发布的 `0.1.19` macOS release 时，必须同时使用 archive、严格 aggregate manifest、两个 Sigstore bundle，以及从独立可信渠道取得的 signing-certificate SHA-256：

```sh
./installer/install-macos.sh \
  --archive agent-remote-ego-browser-macos-universal-0.1.19.tar.gz \
  --archive-sigstore-bundle agent-remote-ego-browser-macos-universal-0.1.19.tar.gz.sigstore.json \
  --manifest agent-remote-ego-browser-0.1.19.release-manifest.json \
  --manifest-sigstore-bundle agent-remote-ego-browser-0.1.19.release-manifest.json.sigstore.json \
...
curl -fsSL https://raw.githubusercontent.com/Agent-Remote/agent-remote-ego-browser/main/scripts/install.sh | \
  bash -s -- --version 0.1.19 --confirm-local-trust
```
...
只有生成的 manifest 为 `production_ready=true` 且无 readiness blocker 时，workflow 才发布 stable release；否则发布 prerelease。`0.1.12` 已按 `community-local-trust` profile 通过组件门禁。剩余部署/canary 门禁与不可变 rollback 合同见[发布、升级与回滚](docs/release.zh-CN.md)。
